// 🏗️ USER EDITABLE - PRICING PAGE CLIENT COMPONENT
// Interactive pricing page with navigation and loading states.
...
export default function PricingPageClient() {
  ...
  const handlePlanClick = (planName: string, href: string) => {
    setIsNavigating(true);
    setClickedPlan(planName);

    // Determine the actual destination
    const destination = isAuthenticated && href === "/auth" ? "/product" : href;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
          <div className="text-center">
            ...
            <p className="text-lg font-semibold text-foreground">
              {clickedPlan === "Pro" || clickedPlan === "Free"
                ? isAuthenticated
                ...
                >
                  ...
                  >
                    ...
                    <CardFooter className="relative pt-10 pb-8 px-8">
                      <Button
                        onClick={() => handlePlanClick(plan.name, plan.ctaHref)}
                        disabled={isNavigating}
                        className={`w-full py-4 text-lg font-bold rounded-xl transition-all duration-300 transform hover:scale-105 disabled:opacity-75 disabled:cursor-not-allowed disabled:transform-none ${
                          plan.popular
                      ...
                      >
                        {isNavigating && clickedPlan === plan.name ? (
                          <>
            ...
            <div className="bg-gradient-to-r from-gray-900 via-emerald-900 to-teal-900 rounded-3xl p-12 text-center text-white relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                  <Button
                    onClick={() => handlePlanClick("Bottom CTA", "/auth")}
                    disabled={isNavigating}
                    className="bg-white text-gray-900 hover:bg-gray-100 px-8 py-4 text-lg font-bold rounded-xl shadow-xl hover:shadow-2xl transform hover:scale-105 transition-all duration-200 disabled:opacity-75 disabled:cursor-not-allowed disabled:transform-none"
                  >
                    {isNavigating && clickedPlan === "Bottom CTA" ? (
                      <>
                        <Loader2 className="mr-2 h-5 w-5 animate-spin" />
                        Loading...
                      </>
